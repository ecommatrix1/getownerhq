export default async function handler(req: any, res: any) {
  ...
  try {
    const { gym_id, plan_name, amount, owner_name, owner_email, owner_mobile } = req.body || {};

    if (!gym_id || !plan_name || !amount) {
      return res.status(400).json({ success: false, message: 'Missing required parameters (gym_id, plan_name, amount)' });
    }
    ...
    if (!clientId || !clientSecret) {
      return res.status(500).json({
        success: false,
        message: 'Cashfree API credentials (CASHFREE_CLIENT_ID / CASHFREE_CLIENT_SECRET) are not configured on server environment variables.'
      });
    }
    ...
    const growthPlanId = process.env.CASHFREE_GROWTH_PLAN_ID || process.env.VITE_CASHFREE_GROWTH_PLAN_ID || 'getownerhq_growth_999';
    const starterPlanId = process.env.CASHFREE_STARTER_PLAN_ID || process.env.VITE_CASHFREE_STARTER_PLAN_ID || 'getownerhq_starter_499';
    const planCode = plan_name.toLowerCase().includes('growth') ? growthPlanId : starterPlanId;

    const subscriptionId = `sub_${gym_id.slice(0, 8)}_${Date.now()}`;
    const cleanPhone = (owner_mobile || '9999999999').replace(/\D/g, '').slice(-10);

    console.log(`[Cashfree API] Environment: ${mode} (${baseUrl}/subscriptions)`);
    console.log(`[Cashfree API] Target Plan ID: "${planCode}" for Plan: "${plan_name}"`);
    ...
    if (!response.ok || (!data.subscription_session_id && !data.subscription_id)) {
      console.error('Cashfree subscription creation error:', data);
      return res.status(response.status || 400).json({
        success: false,
        message: data.message || 'Failed to create Cashfree subscription session',
        details: data,
    ...
    return res.status(500).json({ success: false, message: error.message || 'Server error creating Cashfree subscription' });
  }
}
